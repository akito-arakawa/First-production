...
/* -------------------------------------------------------------------------- */
/* 関数名：Cat_Push                                                           */
/* 機  能：猫がクリックされたときに画像を大きくして吠える                     */
/* 戻り値：なし                                                               */
/* 引  数：なし                                                               */
/* -------------------------------------------------------------------------- */
void Cat_Push_ans(int n, int *p)
{
    ...
    int CatSound;
    CatSound = LoadSoundMem("猫の鳴き声.mp3");
    ...
    //---------------- - 円-------------------------- /
    DrawCircle(1260, 250, 200, Cr2);     //右上の円
    ...
    DrawRotaGraph(1260, 250, 2.0, 0.0, cat, TRUE); //猫の画像


    if (*p == 0) {
        ChangeVolumeSoundMem(80, CatSound);
        PlaySoundFile("猫の鳴き声.mp3", DX_PLAYTYPE_BACK);     //犬の音声

    }
    else if (*p == 1) {
        ChangeVolumeSoundMem(0, CatSound);
    }
