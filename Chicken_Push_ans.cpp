...
/* -------------------------------------------------------------------------- */
/* 関数名：Chicken_Push                                                       */
/* 機  能：鶏がクリックされたときに画像を大きくして吠える                     */
/* 戻り値：なし                                                               */
/* 引  数：なし                                                               */
/* -------------------------------------------------------------------------- */
void Chicken_Push_ans(int n, int *p)
{
    ...
    int  ChickenSound;
    ChickenSound = LoadSoundMem("ニワトリの鳴き声.mp3");
    ...
    DrawCircle(660, 830, 200, Cr2);      //左下の円
    ...
    DrawRotaGraph(660, 830, 2.0, 0.0, chicken, TRUE); // 鶏の画像


    if (*p == 0) {
        ChangeVolumeSoundMem(80, ChickenSound);
        PlaySoundFile("ニワトリの鳴き声.mp3", DX_PLAYTYPE_BACK);     //犬の音声

    }
    else if (*p == 1) {
        ChangeVolumeSoundMem(0, ChickenSound);
    }
