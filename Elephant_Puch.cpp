...
/* -------------------------------------------------------------------------- */
/* 関数名：Elephant_Puch                                                      */
/* 機  能：像がクリックされたときに画像を大きくして吠える                     */
/* 戻り値：なし                                                               */
/* 引  数：int n:                                                             */
/* 引  数：なし                                                               */
/* -------------------------------------------------------------------------- */
void Elephant_Puch(int n, int *p)
{
    ...
    int ElephantSound;
    ElephantSound = LoadSoundMem("ゾウの鳴き声.mp3");
    ...
    DrawCircle(1260, 830, 200, Cr2);     //右下の円
    ...
    DrawRotaGraph(1260, 830, 1.4, 0.0, elephant, TRUE); //像の画像

    if (*p == 0) {
        ChangeVolumeSoundMem(80, ElephantSound);
        PlaySoundFile("ゾウの鳴き声.mp3", DX_PLAYTYPE_BACK);     //犬の音声
    }
    else if (*p == 1) {
        ChangeVolumeSoundMem(0, ElephantSound);
    }
